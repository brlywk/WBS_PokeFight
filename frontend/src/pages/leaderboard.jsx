...
const Leaderboard = () => {
  ...
  return (
    ...
    <div className="flex h-full w-full flex-col items-center justify-start ">
      <Link
        to="/"
        className="continue-button font-['Press_Start_2P'] text-xl text-white font-bold mt-5 text-shadow"
      >
        &lt; Back
      </Link>
      <h1 className="font-['Press_Start_2P'] text-4xl text-white font-bold mt-8 text-shadow">
        Leaderboard
      </h1>
      {fightsLoading && <div>Loading...</div>}
      ...
      {!fightsLoading && fights?.length > 0 && (
        <div className="my-4 w-3/5">
          {fights.map((f) => (
            <div
              key={f._id}
              className="my-4 flex flex-col bg-white/50 p-4 rounded-xl backdrop-blur"
            >
              <div className="flex justify-between mb-4">
                <div>{new Date(f.createdAt).toLocaleDateString()}</div>
                <div>{new Date(f.createdAt).toLocaleTimeString()}</div>
              </div>
              <div className="grid grid-cols-[1fr_1fr_max-content_1fr_1fr] gap-x-4">
                ...
                <div className="font-['Press_Start_2P'] flex items-center justify-center text-4xl text-red-600">
                  VS
                </div>
                <div className="flex flex-col items-center">
                  <img
                ...
                <div className="flex flex-row flex-wrap w-3/4 gap-2">
                  {f.rounds?.length > 0 &&
                    f.rounds.map((rd) => (
                      <div
                        key={rd.round}
                        className="grid grid-cols-[max-content_1fr] gap-x-16 gap-y-2"
                      >
                        ...
                        <div className="flex flex-row flex-wrap">
                          <div className="w-full">
                            <div className="grid grid-cols-[1fr_10fr] gap-x-8">
                              <div className="italic">{f.player_one_name}</div>
                              <div>
                          ...
                          <div className="w-full">
                            <div className="grid grid-cols-[1fr_10fr] gap-x-8">
                              <div className="italic">{f.player_two_name}</div>
                              <div>
